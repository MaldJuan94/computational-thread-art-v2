...
def create_rectangle_nail_positions(shape, nail_step=2):
    ...
    output_niels = {"nails_top": len(nails_top), "nails_bot": len(nails_bot), "nails_right": len(nails_right),
                    "nails_left": len(nails_left), "nails": np.array(nails), }
...
def create_art(nails, orig_pic, str_pic, str_strength, i_limit=None, progress_listener: Callable = None):
    ...
    while True:
        ...
        if i % 500 == 0:
            if progress_listener is not None:
                progress_listener.onProgressUpdate("iteration", i)
            print(f"Iteration {i}")
    ...
    if progress_listener is not None:
        progress_listener.onProgressUpdate("end_create_art", -1)
...
def convert_nails_to_sides(nails_top, nails_bot, nails_right, nails_left):
    top_points = {i: 1 for i in range(nails_top)}
    right_points = {i: 0 for i in range(nails_top, nails_top + nails_bot)}
    bottom_points = {i: 3 for i in range(nails_top + nails_bot, nails_top + nails_bot + nails_right)}
    left_points = {i: 2 for i in
                   range(nails_top + nails_bot + nails_right, nails_top + nails_bot + nails_right + nails_left)}
...
def kaspar_main(args, progress_listener: Callable = None):
    ...
    if args.rgb:
        ...
        if args.rect:
            shape = "rectangle"
            sides = convert_nails_to_sides(nails['nails_top'], nails['nails_bot'], nails['nails_right'],
                                           nails['nails_left'])
        else:
            shape = "circle"
            sides = None
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', action="store", type=int, dest="side_len", default=900)
